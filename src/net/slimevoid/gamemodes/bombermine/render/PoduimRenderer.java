...
public class PoduimRenderer extends TileEntitySpecialRenderer {
	...
	public void renderTileEntityAt(TileEntity tileentity, double x, double y, double z, float f) {
        ...
        EntityPlayer player = Minecraft.getMinecraft().theWorld.getPlayerEntityByName(list.get(metadata).name);
        
        if(player == null) {
        	return;
        }
        
		float pPrevYawHead = player.prevRotationYawHead, pPrevYawO = player.prevRenderYawOffset;
		
		player.prevRotationYawHead = 270;
		player.prevRenderYawOffset = 270;
		
		float pPrevPitch = player.prevRotationPitch;
		float pPrevSwing = player.prevSwingProgress;
...
//		player.field_704_R = 0;
		...
		player.prevRotationYawHead = pPrevYawHead;
